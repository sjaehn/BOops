...
#ifndef OPTIONBITCRUSH_HPP_
#define OPTIONBITCRUSH_HPP_
...
class OptionBitcrush : public OptionWidget
{
public:
		...
		bitLabel (80, 90, 80, 20, "ctlabel", BOOPS_LABEL_BITS)
	{
		try
		{
			options[0] = new DialRange (10, 20, 60, 60, "pad0", 0.5, 0.0, 1.0, 0.0, BIDIRECTIONAL, "%1.2f", "", [] (double x) {return 0.01 + 1.99 * x;}, [] (double x) {return (LIMIT (x, 0.01, 2.0) - 0.01) / 1.99;});
			options[1] = new BWidgets::ValueWidget (0, 0, 0, 0, "widget", 0.0);
			options[2] = new DialRange (90, 20, 60, 60, "pad0", 0.5, 0.0, 1.0, 0.0, BIDIRECTIONAL, "%1.0f", "", [] (double x) {return LIMIT (int (1.0 + 32.0 * x), 1, 32);}, [] (double x) {return (LIMIT (x, 1, 32) - 1.0) / 32.0;});
	...
	OptionBitcrush (const OptionBitcrush& that) : OptionWidget (that), limitLabel (that.limitLabel), bitLabel (that.bitLabel)
	{
		add (limitLabel);
		add (bitLabel);
	}

	OptionBitcrush& operator= (const OptionBitcrush& that)
	{
		release (&limitLabel);
		release (&bitLabel);
		OptionWidget::operator= (that);
		limitLabel = that.limitLabel;
		bitLabel = that.bitLabel;
		add (limitLabel);
		add (bitLabel);
	...
	virtual Widget* clone () const override {return new OptionBitcrush (*this);}
	...
	virtual void applyTheme (BStyles::Theme& theme, const std::string& name) override
	{
		...
		limitLabel.applyTheme (theme);
		bitLabel.applyTheme (theme);
	}
...
protected:
	BWidgets::Label limitLabel;
	BWidgets::Label bitLabel;


};

#endif /* OPTIONBITCRUSH_HPP_ */
