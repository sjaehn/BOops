...
MinusButton::MinusButton () :
		MinusButton (0.0, 0.0, BWIDGETS_DEFAULT_BUTTON_WIDTH, BWIDGETS_DEFAULT_BUTTON_HEIGHT, "upbutton", 0.0) {}

MinusButton::MinusButton (const double x, const double y, const double width, const double height, const std::string& name, double defaultValue) :
		Button (x, y, width, height, name, defaultValue) {}

Widget* MinusButton::clone () const {return new MinusButton (*this);}

void MinusButton::draw (const BUtilities::RectArea& area)
{
	...
	if ((getWidth () >= 6) && (getHeight () >= 6))
	{
		...
		if (cairo_status (cr) == CAIRO_STATUS_SUCCESS)
		{
			...
			cairo_move_to (cr, x0 + w/2 - 0.375 * size, y0 + h/2);
