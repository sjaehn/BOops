...
#ifndef OPTIONGALACTIC_HPP_
#define OPTIONGALACTIC_HPP_
...
class OptionGalactic : public OptionWidget
{
public:
	OptionGalactic () : OptionGalactic (0.0, 0.0, 0.0, 0.0, "widget") {}
	OptionGalactic (const double x, const double y, const double width, const double height, const std::string& name) :
		OptionWidget (x, y, width, height, name),
		replaceLabel (0, 90, 80, 20, "ctlabel", BOOPS_LABEL_REPLACE),
		brightnessLabel (80, 90, 80, 20, "ctlabel", BOOPS_LABEL_BRIGHTNESS),
		detuneLabel (160, 90, 80, 20, "ctlabel", BOOPS_LABEL_DETUNE),
		bignessLabel (240, 90, 80, 20, "ctlabel", BOOPS_LABEL_BIGNESS)
	{
		try
		{
	...
	OptionGalactic (const OptionGalactic& that) :
		OptionWidget (that),
		replaceLabel (that.replaceLabel),
		brightnessLabel (that.brightnessLabel),
		detuneLabel (that.detuneLabel),
		bignessLabel (that.bignessLabel)
	{
		add (replaceLabel);
		add (brightnessLabel);
		add (detuneLabel);
	...
	OptionGalactic& operator= (const OptionGalactic& that)
	{
		release (&replaceLabel);
		release (&brightnessLabel);
		release (&detuneLabel);
		release (&bignessLabel);
	...
	virtual Widget* clone () const override {return new OptionGalactic (*this);}
	...
	virtual void applyTheme (BStyles::Theme& theme, const std::string& name) override
	{
		...
		replaceLabel.applyTheme (theme);
		brightnessLabel.applyTheme (theme);
		detuneLabel.applyTheme (theme);
		bignessLabel.applyTheme (theme);
	}
...
protected:
	BWidgets::Label replaceLabel;
	BWidgets::Label brightnessLabel;
	BWidgets::Label detuneLabel;
	BWidgets::Label bignessLabel;
};

#endif /* OPTIONGALACTIC_HPP_ */
