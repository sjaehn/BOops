...
#ifndef OPTIONAMP_HPP_
#define OPTIONAMP_HPP_
...
class OptionAmp : public OptionWidget
{
public:
		...
		ampLabel (10, 90, 60, 20, "ctlabel", "Amp")
	{
		try
		{
			options[0] = new DialRange (10, 20, 60, 60, "pad0", 0.5, 0.0, 1.0, 0.0, BIDIRECTIONAL, "%1.2f", "", [] (double x) {return 2 * x;}, [] (double x) {return 0.5 * LIMIT (x, 0, 2);});
	...
	OptionAmp (const OptionAmp& that) : OptionWidget (that), ampLabel (that.ampLabel)
	{
		add (ampLabel);
	}

	OptionAmp& operator= (const OptionAmp& that)
	{
		release (&ampLabel);
	...
	virtual Widget* clone () const override {return new OptionAmp (*this);}
